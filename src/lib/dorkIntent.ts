// dorkIntent.ts — precise trigger for Aureon's Asherin-Engine dork battery.
//
// Narrative → Flaws → New narrative:
//   "Trigger on 'find'" is a trap — "find me coffee" would fire the whole
//   OSINT battery. The rebuilt rule REQUIRES a dork verb AND either an
//   explicit dork-domain marker (domain, org, real name, "expose", "leak",
//   "audit exposure", "public about", "dork") OR the word "dork" itself.
...
function looksLikeProperName(text: string): string | null {
  // 2–4 capitalized tokens, e.g. "Jane Doe" or "Acme Robotics"
...
export function detectDorkIntent(userText: string): DorkTrigger {
  ...
  if (!text || text.length < 4) {
    return { fire: false, subject: "", kind: "topic", hints: {}, reason: "empty" };
  }

  const hard = HARD_TRIGGERS.some((r) => r.test(text));
  const softVerb = SOFT_VERBS.test(text);
  const softObj = SOFT_OBJECTS.test(text);
  const softFire = softVerb && softObj;

  if (!hard && !softFire) {
    return { fire: false, subject: "", kind: "topic", hints: {}, reason: "no_trigger" };
  }
  ...
  if (domain) { subject = domain; kind = "domain"; }
    ...
    // fallback: strip trigger noise, use remaining as topic
    const cleaned = text.replace(/\b(please|can you|could you|hey aureon|aureon)\b/gi, "").trim();
    if (cleaned.length < 4) {
      return { fire: false, subject: "", kind: "topic", hints: {}, reason: "no_subject" };
    }
