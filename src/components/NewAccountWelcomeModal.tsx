  ...
  return (
    <div className="p-7 space-y-5 max-h-[80vh] overflow-y-auto">
      ...
      <p className="text-sm font-extralight leading-relaxed text-muted-foreground">
        ...
        see subscriptions. $18 / month. $79 / month pro. this is the offer. it does not claim a named hosted model is
      ...
      <div className="pt-1">
        <p className="font-mono text-[10px] tracking-[0.3em] uppercase text-foreground/40 mb-2">◈ Plans</p>
        <PlanList compact />
      </div>
...
function PlanList({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`grid grid-cols-1 ${compact ? "sm:grid-cols-3" : "sm:grid-cols-3"} gap-2`}>
      {PLANS.map((p) => (
        <div key={p.tier} className="rounded-md border border-white/10 bg-white/[0.015] px-3 py-2">
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className="text-sm font-light text-foreground tabular-nums">
              {p.price}
              <span className="text-[10px] text-muted-foreground">{p.per}</span>
            </span>
          </div>
          {!compact && <p className="mt-1 text-[11px] font-extralight leading-snug text-muted-foreground">{p.desc}</p>}
        </div>
      ))}
    </div>
  );
