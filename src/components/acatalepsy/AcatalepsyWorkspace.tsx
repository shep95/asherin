...
import { Archive, ClipboardPaste, Download, Eraser, FileDown, FolderOpen, Globe2, Link2, Search, ShieldCheck, Upload, X } from "lucide-react";
...
export default function AcatalepsyWorkspace() {
  ...
  const exportPdf = async () => { const node = document.querySelector("[data-chart-export]") as HTMLElement | null; if (!node || !active) return; const canvas = await (await import("html2canvas")).default(node, { backgroundColor: "#080808", scale: 1.5 }); const { jsPDF } = await import("jspdf"); const pdf = new jsPDF({ orientation: "landscape", unit: "px", format: [canvas.width, canvas.height] }); pdf.addImage(canvas.toDataURL("image/png"), "PNG", 0, 0, canvas.width, canvas.height); pdf.save(`${active.file.name}-visual.pdf`); };
