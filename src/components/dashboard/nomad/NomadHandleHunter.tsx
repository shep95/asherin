...
const NomadHandleHunter = ({ entities }: NomadHandleHunterProps) => {
  ...
  // Handle hunt - generates platform pivot results
...
Also note any patterns: handle reuse across platforms, naming conventions, and potential sock puppets.`,
      ...
      const results: PlatformResult[] = PLATFORMS.map(p => ({
        platform: p,
        url: `https://${p.toLowerCase().replace(/[^a-z]/g, "")}.com/${cleanHandle}`,
        confidence: Math.floor(Math.random() * 60 + 20),
        falsePositiveRisk: (["low", "medium", "high"] as const)[Math.floor(Math.random() * 3)],
      }));

      const search: HandleSearch = { id: crypto.randomUUID(), handle: cleanHandle, results, timestamp: Date.now() };
  ...
  const translate = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setTranslatedResult(typeof data === "string" ? data : data?.choices?.[0]?.message?.content || JSON.stringify(data));
    } catch (e) {
...
List all variants clearly.` },
      ...
      setVariants(prev => [...prev, { original: variantName, variants: content.split("\n").filter((l: string) => l.trim()), script: "multi" }]);
    } catch (e) {
  ...
  // Auto-populate handles from entities
            ...
            <div className="space-y-4">
              ...
              {searches.map(s => (
                <div key={s.id} className="rounded-xl border border-border/20 bg-card/10 p-4 space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {s.results.sort((a, b) => b.confidence - a.confidence).map((r, i) => (
                      <div key={i} className="flex items-center gap-2 rounded-lg border border-border/10 bg-card/5 px-3 py-1.5">
              ...
              {searches.length === 0 && !loading && (
                <div className="text-center py-8">
                  ...
                  <p className="text-[11px] text-muted-foreground/40 font-light">Enter a username to pivot across 18+ platforms with confidence scoring.</p>
                </div>
              )}
            </div>
          )}
