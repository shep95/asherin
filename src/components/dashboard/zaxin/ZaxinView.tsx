              ...
              {(["full","face","fingers"] as BodyMode[]).map((m) => (
                <button key={m} onClick={() => toggleMode(m)}
                  className={`px-2.5 py-1 rounded-full text-[10px] tracking-[0.06em] transition ${
                    modes.has(m)
                      ? "bg-#c69a4a/15 text-emerald-200 border border-#c69a4a/40"
          ...
          <div className="ml-auto flex items-center gap-1.5">
            {props.arOn && (
              <span className={`text-[9px] tracking-[0.16em] font-mono px-2 py-0.5 rounded-full ${
                ...
                bvReady ? "text-emerald-200 bg-#c69a4a/10" :
...
function IconChip({ icon: Icon, onClick, active, tone, label }: {
  ...
  const cls = tone === "danger"
    ...
    : active
      ? "border-#c69a4a/40 text-emerald-200 bg-#c69a4a/10"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 pointer-events-none">
            <div className="pointer-events-auto flex flex-col items-center gap-2 px-3 py-2 rounded-lg bg-black/55 border border-#c69a4a/15 backdrop-blur-sm">
              ...
              {mode !== "unsupported" && (
                <button
                  onClick={onPick}
                  className="mt-0.5 px-2.5 py-1 rounded-md text-[9px] tracking-[0.18em] uppercase border border-#c69a4a/30 text-emerald-200/90 hover:bg-#c69a4a/[0.06]"
                >+ Add Device</button>
              )}
            </div>
          </div>
      ...
      {!compassOn ? (
        <div className="flex flex-col items-center gap-1">
          <button
            ...
            className="px-3 py-1.5 rounded-md text-[10px] tracking-[0.18em] uppercase border border-#c69a4a/30 text-emerald-200/90 hover:bg-#c69a4a/[0.06]"
