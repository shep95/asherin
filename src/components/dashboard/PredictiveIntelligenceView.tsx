  ...
  const generatePredictions = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({ title: "Predictions Generated", description: `Found ${result.count} potential events for ${companyInput}` });
      ...
      <div className="flex-shrink-0 p-4 sm:p-6 border-b border-border/20 bg-card/20 backdrop-blur-sm space-y-4">
        ...
        {showSettings && (
          <div className="rounded-xl border border-purple-500/20 bg-card/30 backdrop-blur-sm p-4 space-y-3">
            <p className="text-xs text-muted-foreground">Enter a company name to scan for predictive signals across the web.</p>
            ...
            {generating && (
              <div className="flex items-center gap-2 text-xs text-purple-400/70">
                ...
                Searching web sources, detecting signals, and generating predictions... This may take 30-60 seconds.
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex flex-col items-center justify-center flex-1 gap-4 p-8">
          ...
          <div className="text-center max-w-md space-y-3">
            ...
            <p className="text-xs font-extralight leading-relaxed text-muted-foreground/70">
              Generate AI-powered predictions by scanning real-time web signals. Enter a company name above to forecast regulatory actions, executive departures, earnings surprises, and more.
            </p>
            <button
            ...
            {filtered.map(prediction => {
              ...
              return (
                <div key={prediction.id} className={`rounded-xl border transition-all ${config.border}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            ...
                            {prediction.status === "confirmed" && (
                              <span className="px-2 py-0.5 rounded text-[9px] font-medium uppercase tracking-wider border border-emerald-500/30 bg-emerald-500/20 text-emerald-400">
                                ✓ Confirmed
                              </span>
                            )}
                          </div>
                          ...
                          {/* Prediction text */}
                          <p className="text-sm font-light text-foreground leading-relaxed mb-3">
                          ...
                          <div className="flex items-center gap-4 text-[10px] text-muted-foreground/60 flex-wrap">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{prediction.signals?.length || 0} signals</span>
                            </div>
                            <div className="flex items-center gap-1">
                      ...
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        <div className="text-right">
                          <div className={`text-2xl font-light font-mono ${
                            ...
                            prediction.confidence > 0.6 ? "text-amber-400" : "text-red-400"
                          }`}>
                  ...
                  {/* Expanded details */}
                  {isExpanded && (
                    <div className="border-t border-border/10 p-4 space-y-5 bg-background/20">
                      {/* Signals */}
                      {prediction.signals && prediction.signals.length > 0 && (
                        <div>
                          <h4 className="text-[10px] font-medium text-muted-foreground/60 uppercase tracking-wider mb-3">
                            Contributing Signals ({prediction.signals.length})
                          </h4>
                          <div className="space-y-2">
                            {prediction.signals.slice(0, 8).map((signal: any, idx: number) => (
                              <div key={idx} className="flex items-start gap-3 p-3 rounded-lg border border-border/10 bg-card/20 hover:bg-card/30 transition-colors">
                                <div className="flex-shrink-0 w-10 text-[10px] font-mono text-muted-foreground/50 text-right pt-0.5">
                                  {((signal.weight || 0) * 100).toFixed(0)}%
                                </div>
                                <div className="flex-1 min-w-0">
                                ...
                                <div className="flex flex-col gap-1 text-[10px] text-muted-foreground/40">
                                  <div className="flex items-center gap-1">
                                    <span className={`w-1.5 h-1.5 rounded-full ${
                                      (signal.scores?.relevance || 0) > 0.7 ? "bg-emerald-400" :
                                      (signal.scores?.relevance || 0) > 0.5 ? "bg-amber-400" : "bg-red-400"
                                    }`} />
                                    <span className="font-mono">{signal.scores?.relevance ? (signal.scores.relevance * 100).toFixed(0) : "?"}%</span>
                                  </div>
                                </div>
                              </div>
                            ))}
                            ...
                            {prediction.reasoning_chain.map((step: any, idx: number) => (
                              <div key={idx} className="flex gap-3">
                                ...
                                <div className="flex-1 pb-1">
                                  ...
                                  <p className="text-[10px] text-muted-foreground/50">{step.output}</p>
