...
const exportDetailAsTxt = (detail: PatternDetail, category: string, recordDrill: RecordDrillDown | null, selectedRecord: DetailRecord | null) => {
  let txt = "";
  txt += "╔═══════════════════════════════════════════════════════════════╗\n";
  txt += "║     ASHERIN COMPLETE WASTE INTELLIGENCE REPORT                 ║\n";
  ...
  if (selectedRecord && recordDrill) {
    ...
    if (recordDrill.asherinRecommendation) txt += `ASHERIN STRATEGIC RECOMMENDATION:\n${recordDrill.asherinRecommendation}\n\n`;
  ...
  txt += "\n╔═══════════════════════════════════════════════════════════════╗\n";
  txt += "║  END OF REPORT — Generated by Asherin Financial Intelligence  ║\n";
  txt += "╚═══════════════════════════════════════════════════════════════╝\n";
  ...
  a.href = url;
  a.download = `asherin-forensic-intelligence-${Date.now()}.txt`;
  a.click();
...
const ZeeionDeepDive = ({ category, context, columnHint, label }: DeepDiveProps) => {
  ...
  const detailCacheKey = `asherin_deepdive_live_v3_${category.replace(/\s+/g, "_")}`;
  const drillCacheKey = (recId: string) => `asherin_drill_live_v3_${category.replace(/\s+/g, "_")}_${recId}`;
    ...
    try {
      const unsupportedDrill: RecordDrillDown = {
        ...
        contributingFactors: [
          ...
          "Asherin will not infer or invent actors, companies, approval chains, or case evidence.",
        ],
        evidence: [
        ...
        whyHarmful: "Asherin cannot assert case-specific harm without source-backed record evidence.",
        potentialEffects: ["Connect live contract, payroll, spending, or audit records to enable real evidence-backed analysis."],
        asherinRecommendation: "Connect a verifiable public record source before using this panel for case-level accusations or named attribution.",
        responsibleParty: {
              ...
              <div className="space-y-4 mt-2">
                ...
                {!drillLoading && recordDrill && (
                  <div className="space-y-4">
                    ...
                    {/* Asherin Recommendation */}
                    {recordDrill.asherinRecommendation && (
                      <div className="rounded-xl border border-emerald-500/10 bg-emerald-500/[0.03] p-4">
                        <p className="text-[8px] uppercase tracking-[0.15em] text-emerald-400/50 mb-1 flex items-center gap-1.5"><Target className="h-3 w-3" /> Asherin Strategic Recommendation</p>
                        <p className="text-[10px] text-foreground/60 font-light leading-relaxed">{recordDrill.asherinRecommendation}</p>
                      </div>
                    )}
