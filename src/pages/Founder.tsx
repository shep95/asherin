    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div id="ebook" className="relative z-10 px-6 py-28 sm:py-36 scroll-mt-24">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            {[
              ...
              <div key={b.title} className="rounded-2xl border border-border/30 bg-card/40 backdrop-blur-md p-6 sm:p-8 flex flex-col">
                ...
                <p className="mt-4 text-base font-extralight leading-[1.7] text-foreground/75 flex-1">{b.desc}</p>
                <div className="mt-6 flex flex-wrap gap-3">
                  <a
                    href={b.pdf}
                    download
                    className="inline-flex items-center gap-2 rounded-xl border border-foreground/30 bg-foreground/10 px-5 py-3 text-xs font-light tracking-[0.18em] text-foreground uppercase transition-all hover:bg-foreground/20"
                  >
                  ...
                  <a
                    href={`#${b.anchor}`}
                    onClick={(e) => { e.preventDefault(); document.getElementById(b.anchor)?.scrollIntoView({ behavior: "smooth" }); }}
                    className="inline-flex items-center gap-2 rounded-xl border border-border/30 bg-background/30 px-5 py-3 text-xs font-light tracking-[0.18em] text-foreground/80 uppercase transition-all hover:bg-foreground/10"
                  >
          ...
          <div id="book-2" className="rounded-2xl border border-border/20 bg-card/30 backdrop-blur-md p-6 sm:p-8 scroll-mt-24">
            <h3 className="text-lg font-light tracking-wide text-foreground mb-5">Symbolism & More · Reader</h3>
            <div className="max-h-[85vh] overflow-y-auto rounded-xl border border-border/20 bg-background/40 p-3 sm:p-5">
              <div className="mx-auto flex max-w-3xl flex-col gap-5">
                {symbolismBookPages.map((pageSrc, index) => (
                  <img
                    key={pageSrc}
                    src={pageSrc}
                    alt={`Symbolism & More page ${index + 1}`}
                    loading={index < 2 ? "eager" : "lazy"}
                    className="w-full rounded-lg border border-border/20 bg-background shadow-2xl shadow-black/30"
                  />
                ))}
